...
function EditRoadmapPage() {
    ...
    useEffect(() => {
        const fetchRoadmap = async () => {
            try {
                const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/api/roadmaps/${id}`);
                setTopic(data.topic);
    ...
    const submitHandler = async (e) => {
        ...
        try {
            ...
            const updatedRoadmap = { topic, category, description, difficulty, steps };
            await axios.put(`${import.meta.env.VITE_API_URL}/api/roadmaps/${id}`, updatedRoadmap, config);
            if (user.isAdmin) {
    ...
    return (
        <div className="container mx-auto max-w-3xl py-12 px-4">
            <h1 className="text-3xl font-bold text-center mb-6">Edit Roadmap</h1>
            <form onSubmit={submitHandler} className="space-y-6 bg-white dark:bg-slate-800 p-8 shadow-md rounded-lg">
                ...
                <div className="space-y-4">
                    {steps.map((step, stepIndex) => (
                        <div key={stepIndex} className="p-4 border rounded-md bg-slate-50 dark:bg-slate-700">
                ...
                <button type="button" onClick={addStep} className="w-full border-2 border-dashed p-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700">+ Add Step</button>
                <hr className="border-slate-300 dark:border-slate-600"/>
                <button type="submit" className="w-full bg-cyan-600 text-white p-3 rounded-md hover:bg-cyan-700 font-bold">Update Roadmap</button>
            </form>
        </div>
    );
}
